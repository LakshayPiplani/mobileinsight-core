    ...
    case 24:
        {
            ...
            _replace_result_int(result, "Serving Cell ID",
                    iServingCellId);
            ...
            _replace_result_int(result, "UE Category",
                    iUECategory);
            _replace_result_int(result, "Num DL HARQ",
                    iNumDlHarq);
            _replace_result_int(result, "TM Mode",
                    iTmMode);
            ...
            for (int i = 0; i < num_record; i++) {
                ...
                FieldList result_record_stream;
                for (int j = 0; j < num_stream; j++) {
                    FieldList result_record_stream_item;
                    ...
                    FieldList result_energy_metric;
                    for (int k = 0; k < num_energy_metric; k++) {
                        FieldList result_energy_metric_item;
                        ...
                        result_energy_metric.push_back({"Ignored", FieldValue{std::move(result_energy_metric_item)}, "dict"});
                    }
                    offset += (13 - num_energy_metric) * 4;

                    result_record_stream_item.push_back({"Energy Metrics", FieldValue{std::move(result_energy_metric)}, "list"});

                    result_record_stream.push_back({"Ignored", FieldValue{std::move(result_record_stream_item)}, "dict"});
                }

                result_record_item.push_back({"Streams", FieldValue{std::move(result_record_stream)}, "list"});
    ...
    case 105:
        {
            ...
            _replace_result_int(result, "TM Mode",
                    iTmMode);
            _replace_result_int(result, "Nir", iNir);
            _replace_result_int(result, "Carrier Index", iCarrierIndex);
            (void) _map_result_field_to_name(result, "Carrier Index",
                    ValueNameCarrierIndex,
                    ARRAY_SIZE(ValueNameCarrierIndex, ValueName),
    ...
    case 124:
        {
            ...
            for (int i = 0; i < num_record; i++) {
                ...
                for (int j = 0; j < iNumofTBlks; j++) {
                    ...
                    temps+=hex;
                    FieldValue pystr1 = FieldValue{std::string(temps.c_str())};
                    _replace_result(result_record_stream_item,"Effective Code Rate", pystr1);
                    ...
                    while ((unsigned int)(offset - start) < length) {
                        FieldList result_hidden_record;
