...
_decode_McePayload_v4(const char* b, int offset, size_t length,
	...
	int type = tmp & 0x3f;
	_replace_result_int(records, "MCE Type", type);
	...
	if (type == 0x3d)//S-BSR
	{
		...
		int index = tmp & 0x1f;
		_replace_result_int(records, "ShortBsr-BSR Index", index);

		int lcg = (tmp >> 5);
		_replace_result_int(records, "ShortBsr-BSR LCG", lcg);
	}
	else if (type == 0x39) {
		...
		int ph = tmp & 0x3f;
		_replace_result_int(records, "SinglePhr-PH", ph);

		tmp= _search_result_int(records, "SinglePhr-Pcmax fc");
		int fc = tmp & 0x3f;
		_replace_result_int(records, "SinglePhr-Pcmax fc", fc);

		int r = tmp>>6;
		_replace_result_int(records, "SinglePhr-R", r);
...
_decode_TB_Info_v4(const char* b, int offset, size_t length,
	...
	int harq = (tmp >> 3) & 0xf;
	_replace_result_int(records, "Harq ID", harq);

	int tmp2 = _search_result_int(records, "Carrier ID");
	int carrier_id = ((tmp2 & 0b1) << 1) + ((tmp >> 7)&0b1);
	_replace_result_int(records, "Carrier ID", carrier_id);

	int tb_type = (tmp2 >> 1) & 0b1111;
	_replace_result_int(records, "TB Type", tb_type);
	...
	(void)_map_result_field_to_name(records,
		...
		"(MI)Unknown");
	int rnti = tmp2 >> 5;
	_replace_result_int(records, "RNTI Type", rnti);
	(void)_map_result_field_to_name(records,
	...
	int s_pdu = tmp & 0b1;
	_replace_result_int(records, "Start PDU Seg", s_pdu);
	(void)_map_result_field_to_name(records,
		"Start PDU Seg", logic,
	...
	_replace_result_int(records, "Num Complete PDU", num_pdu);
	
	int e_pdu = (tmp2 >> 3) & 0b1;
	_replace_result_int(records, "End PDU Seg", e_pdu);
	(void)_map_result_field_to_name(records,
		"End PDU Seg", logic,
		ARRAY_SIZE(logic, ValueName),
		"(MI)Unknown");
    ...
    if (MCE_Bitmask & 0b1)//PHR
	{
		...
		_replace_result_int(records, "PHR Reason", phr_reason);
		(void)_map_result_field_to_name(records,
			"PHR Reason", phr_reason_v4,
			ARRAY_SIZE(phr_reason_v4, ValueName),
			"(MI)Unknown");
		int phr_c = (tmp >> 5) & 0b11;
		_replace_result_int(records, "PHR Cancel Reason", phr_c);
		(void)_map_result_field_to_name(records,
		...
		std::string s = "";
		_replace_result_string(records, "PHR Reason", s);
		_replace_result_string(records, "PHR Cancel Reason", s);
	}
	if (MCE_Bitmask & 0b10) {
		int bsr_reason = ((tmp2 & 0b1) << 2) + ((tmp >> 6) & 0b11);
		_replace_result_int(records, "BSR Reason", bsr_reason);
		int bsr_type = (tmp2 >> 1) & 0b1111;
		_replace_result_int(records, "BSR Type", bsr_type);
		(void)_map_result_field_to_name(records,
		...
		std::string s = "";
		_replace_result_string(records, "BSR Reason", s);
		_replace_result_string(records, "BSR Type", s);
	}
    
	tmp = _search_result_int(records, "MCE Length");
	int mce_l = tmp & 0x3f;
	_replace_result_int(records, "MCE Length", mce_l);
	...
	records.push_back({"McePayload", FieldValue{std::move(records_list)}, "list"});
...
_decode_TTI_Info_v4(const char* b, int offset, size_t length,
	...
	int fn = tmp & 0x3ff;
	_replace_result_int(records, "Sys Time FN", fn);

	tmp = _search_result_int(records, "Num TB");
	int tb = tmp & 0x0f;
	_replace_result_int(records, "Num TB", tb);
	...
	records.push_back({"TB Info", FieldValue{std::move(records_list)}, "list"});
...
_decode_nr_l2_ul_tb_subpkt(const char* b, int offset, size_t length,
	...
	case 4:
		...
		tti_num = tmp;
		value = tmp & 0x0f;
		_replace_result_int(result, "Num TTI", value);

		value = (tmp >> 4) & 0b1;
		_replace_result_int(result, "Is Type2 Scell", value);

		value = (tmp >> 5) & 0b1;
		_replace_result_int(result, "Is Type2 Other Cell", value);
		for (int i = 0; i < tti_num; i++) {
			std::string s = "TTI Info [" + std::to_string(i) + "]";
			offset += _decode_TTI_Info_v4(b, offset, length, tmp_py, s);
		}
		result.push_back({"TTI Info", FieldValue{std::move(tmp_py)}, "list"});
		success = 1;
		break;
