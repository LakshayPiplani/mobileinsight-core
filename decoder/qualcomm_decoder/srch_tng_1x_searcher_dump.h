            ...
            for (int i = 0; i < num_subpkts; i++) {
                ...
                if (subpkt_id == 2 && subpkt_ver == 2) {
                    ...
                    int num_chains = _search_result_int(result_subpkt,
                            "Num Chains");
                    FieldList result_chain;
                    for (int j = 0; j < num_chains; j++) {
                        FieldList result_chain_item;
                        ...
                        _replace_result(result_chain_item,
                                "Rx AGC", FieldValue{double(fRxAgc)});
                        ...
                        result_chain.push_back({"Ignored", FieldValue{std::move(result_chain_item)}, "dict"});
                    }
                    result_subpkt.push_back({"Clients", FieldValue{std::move(result_chain)}, "list"});
                    ...
                    for (int j = 0; j < num_tasks; j++) {
                        ...
                        _replace_result_int(result_task_item,
                                "QOF", iQOF);
                        _replace_result_int(result_task_item,
                                "HDR Burst Len", iHBL);
                        _replace_result_int(result_task_item,
                        ...
                        FieldList result_peak;
                        for (int k = 0; k < num_peaks; k++) {
                            FieldList result_peak_item;
                            offset += _decode_by_fmt(SrchTng1xsd_Peak_v1,
                                    ARRAY_SIZE(SrchTng1xsd_Peak_v1, Fmt),
                                    b, offset, length, result_peak_item);
                            result_peak.push_back({"Ignored", FieldValue{std::move(result_peak_item)}, "dict"});
                        }
                        result_task_item.push_back({"Peaks", FieldValue{std::move(result_peak)}, "list"});

                        result_task.push_back({"Ignored", FieldValue{std::move(result_task_item)}, "dict"});
                    }
                    result_subpkt.push_back({"Tasks", FieldValue{std::move(result_task)}, "list"});
