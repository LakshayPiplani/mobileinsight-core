    ...
    int offset, size_t length, FieldList &result)
{
    ...
    case 1:
    {
        ...
        FieldValue t = FieldValue{std::string(id)};
        _replace_result(result, "PLMN Id", t);

        FieldList guti_result;
        ...
        t = FieldValue{std::string(ue_id)};
        _replace_result(guti_result, "UE Id", t);
...
//            "(MI)Unknown");        
        ...
        t = FieldValue{std::string(guti_plmn_id)};
        _replace_result(guti_result, "GUTI PLMN ID", t);

        char amf_region_id[64];
        sprintf(amf_region_id, "0x%x", tmp & 0xff);
        t = FieldValue{std::string(amf_region_id)};
        _replace_result(guti_result, "AMF Region ID", t);

        char tmsi[64];
        tmp = _search_result_uint(guti_result, "5G TMSI");
        sprintf(tmsi, "{0x%x,0x%x,0x%x,0x%x}", (tmp >> 24)&0xff, (tmp >> 16) & 0xff, (tmp >> 8) & 0xff,tmp&0xff);
        t = FieldValue{std::string(tmsi)};
        _replace_result(guti_result, "5G TMSI", t);
        result.push_back({"GUTI", FieldValue{std::move(guti_result)}, "dict"});
...
//        t = FieldValue{std::string(tac)};
//        _replace_result(result, "NR5G TAC", t);
