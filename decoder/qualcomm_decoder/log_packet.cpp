...
/*
 * The decoding result is represented as a FieldList (decoder/field_value.h) —
 * a vector of FieldEntry {name, value, type_hint}.
 * "type_hint" can be one of the following values:
 * - "" (empty string): an ordinary scalar field (int64_t, double, string).
 * - "list": The value is a nested FieldList, and each element is of the same
 *      type (one "dict" entry per record).
 * - "dict": The value is a nested FieldList whose elements can be of
 *      different types.
 * - "raw_msg/TYPE": The value is std::vector<uint8_t> raw bytes; TYPE names
 *      the dissector for the payload.
 */
...
_decode_wcdma_signaling_messages(const char *b, int offset, size_t length,
    ...
    type_str += ch_name;
    result.push_back({"Msg", FieldValue{std::vector<uint8_t>((const uint8_t*)(b + offset), (const uint8_t*)(b + offset) + (pdu_length))}, type_str.c_str()});
    return offset - start;
}
...
_decode_umts_nas_ota(const char *b, int offset, size_t length,
    ...
    int pdu_length = _search_result_int(result, "Message Length");
    result.push_back({"Msg", FieldValue{std::vector<uint8_t>((const uint8_t*)(b + offset), (const uint8_t*)(b + offset) + (pdu_length))}, "raw_msg/NAS"});
    return pdu_length;
}
...
_decode_lte_nas_plain(const char *b, int offset, size_t length,
    ...
    result.push_back({"Msg", FieldValue{std::vector<uint8_t>((const uint8_t*)(b + offset), (const uint8_t*)(b + offset) + (pdu_length))}, "raw_msg/LTE-NAS_EPS_PLAIN"});
    return length - start;
}
    ...
    switch (pkt_ver) {
        case 2: {
            ...
            if (table != NULL && table_size > 0) {
                ...
                FieldValue pystr = FieldValue{std::string("Undefined")};
...
_decode_lte_phy_pdsch_demapper_config(const char *b, int offset, size_t length,
    ...
    switch (pkt_ver) {
        case 23: {
            ...
            _replace_result_int(result, "Transmission Scheme",
                                             iTransmissionScheme);
            ...
            FieldValue pyfloat = FieldValue{double(ratio)};
        ...
        case 28: {
            ...
            _replace_result_int(result, "Repetition Index Data",
                                             iRepetionIndexData);
            _replace_result_int(result, "Frequency Selective PMI",
                                             iFrequencySelectivePMI);
            (void) _map_result_field_to_name(result, "Frequency Selective PMI",
            ...
            _replace_result_int(result, "Repetition Total", iRepetitionTotal);
            _replace_result_int(result, "NB Index", iNBIndex);
            _replace_result_int(result, "SIB1-BR Collision", iSIB1BRCollision);
            _replace_result_int(result, "SIBx Collision", iSIBxCollision);
        ...
        case 122: {
            ...
            (void) _map_result_field_to_name(result, "Transmission Scheme",
                                             ...
                                             ARRAY_SIZE(ValueNameTransmissionScheme, ValueName),
                                             "(MI)Unknown");
            _replace_result_int(result, "BMOD FD Sym Index",
                                             iBMODFMSymIndex);
            ...
            _replace_result_int(result, "MVC", iMVC);

            _replace_result_int(result, "MVC Clock",
                                             iMVCClock);
            (void) _map_result_field_to_name(result,
                                             "MVC Clock",
            ...
            _replace_result_int(result, "MVC Request Up",
                                             iMVCRequestUp);
            ...
            _replace_result_int(result, "PB", ipb);
            ...
            _replace_result_int(result, "CSI-RS Exist", iCSIRSExist);
            _replace_result_int(result, "ZP CSI-RS Exist", iZPCSIRSExist);
            _replace_result_int(result, "CSI-RS Symbol Skipped", iCSIRSSymbolSkipped);
            ...
            _replace_result_int(result, "Op Mode", iOPMode);
            ...
            _replace_result_int(result, "Strong ICell ID", iStrongICellID);
            _replace_result_int(result, "Joint Demod Skip Reason", iJointDemodSkipReason);
        ...
        case 144: {
            ...
            _replace_result_int(result, "Num of Records", iNumOfRecords);
            ...
            for (int i = 0; i < iNumOfRecords; i++) {
                ...
                _replace_result_int(result_item, "Serving Cell ID", iresulttemp);
                iresulttemp = (utemp >> 9) & 0xf;
                _replace_result_int(result_item, "Sub-frame Number", iresulttemp);
                iresulttemp = (utemp >> 13) & 0x3ff;
                _replace_result_int(result_item, "System Frame Number", iresulttemp);
                iresulttemp = (utemp >> 23) & 0xf;
                _replace_result_int(result_item, "PDSCH RNTI Type", iresulttemp);
                iresulttemp = (utemp >> 27) & 0x3;
                _replace_result_int(result_item, "Number of Tx Antennas (M)", iresulttemp);
                iresulttemp = (utemp >> 29) & 0x3;
                _replace_result_int(result_item, "Number of Rx Antennas (N)", iresulttemp);
                //iresulttemp =(utemp>>31) & 0x1;
                //_replace_result_int(result_item, "Reserved",iresulttemp);
                ...
                _replace_result_int(result_item, "Spatial Rank", iresulttemp);
                //iresulttemp =(utemp>>4) & 0x1;
                //_replace_result_int(result_item, "Reserved",iresulttemp);
                //Py_DECREF(old_object);
                iresulttemp = (utemp >> 5) & 0x1;
                _replace_result_int(result_item, "Frequency Selective PMI", iresulttemp);
                iresulttemp = (utemp >> 6) & 0x3;
                _replace_result_int(result_item, "MU Receiver Mode", iresulttemp);

                utemp = _search_result_uint(result_item, "PMI Index");
                iresulttemp = (utemp >> 0) & 0xf;
                _replace_result_int(result_item, "PMI Index", iresulttemp);
                iresulttemp = (utemp >> 4) & 0xf;
                _replace_result_int(result_item, "Transmission Scheme", iresulttemp);

                utemp = _search_result_uint(result_item, "UERS Port Enabled");
                iresulttemp = (utemp >> 0) & 0x7;
                _replace_result_int(result_item, "UERS Port Enabled", iresulttemp);
                iresulttemp = (utemp >> 3) & 0xf;
                _replace_result_int(result_item, "BMOD FD Sym Index", iresulttemp);
                iresulttemp = (utemp >> 7) & 0x3ffff;
                _replace_result_int(result_item, "Transport Block Size Stream 0", iresulttemp);
                iresulttemp = (utemp >> 25) & 0x3;
                _replace_result_int(result_item, "Modulation Stream 0", iresulttemp);
                iresulttemp = (utemp >> 27) & 0x3;
                _replace_result_int(result_item, "PB", iresulttemp);
                iresulttemp = (utemp >> 29) & 0x3;
                _replace_result_int(result_item, "RhoB/RhoA", iresulttemp);
                iresulttemp = (utemp >> 31) & 0x1;
                _replace_result_int(result_item, "CSI-RS Exist", iresulttemp);

                utemp = _search_result_uint(result_item, "ZP CSI-RS Exist");
                iresulttemp = (utemp >> 0) & 0x1;
                _replace_result_int(result_item, "ZP CSI-RS Exist", iresulttemp);
                iresulttemp = (utemp >> 1) & 0x1;
                _replace_result_int(result_item, "CSI-RS Symbol Skipped", iresulttemp);
                iresulttemp = (utemp >> 2) & 0xfff;
                _replace_result_int(result_item, "Traffic to Pilot Ratio Data", iresulttemp);
                iresulttemp = (utemp >> 14) & 0x3ffff;
                _replace_result_int(result_item, "Transport Block Size Stream 1", iresulttemp);

                utemp = _search_result_uint(result_item, "Modulation Stream 1");
                iresulttemp = (utemp >> 0) & 0x3;
                _replace_result_int(result_item, "Modulation Stream 1", iresulttemp);
                iresulttemp = (utemp >> 2) & 0x3;
                _replace_result_int(result_item, "SCH0 Memory Map Mode", iresulttemp);
                iresulttemp = (utemp >> 4) & 0x3;
                _replace_result_int(result_item, "SCH1 Memory Map Mode", iresulttemp);
                iresulttemp = (utemp >> 6) & 0x1ff;
                _replace_result_int(result_item, "Strong ICell ID", iresulttemp);
                iresulttemp = (utemp >> 15) & 0x3;
                _replace_result_int(result_item, "Qice Enable Mode", iresulttemp);
                iresulttemp = (utemp >> 17) & 0x7;
                _replace_result_int(result_item, "Qice Skip Reason", iresulttemp);
                iresulttemp = (utemp >> 20) & 0x1;
                _replace_result_int(result_item, "Csf Dual Rnn Sel", iresulttemp);
                iresulttemp = (utemp >> 21) & 0x1f;
                _replace_result_int(result_item, "Plq Num Enabled Rd Groups", iresulttemp);
                iresulttemp = (utemp >> 26) & 0x1f;
                _replace_result_int(result_item, "Plg Num Loaded Rb Groups", iresulttemp);
                iresulttemp = (utemp >> 31) & 0x7;
                _replace_result_int(result_item, "Qed Mode", iresulttemp);
                ...
                */
                ...
                result.push_back({"Ignored", FieldValue{std::move(result_item)}, "dict"});
            }
            break;
        }
        default:
    ...
    case 2:
        {
            ...
            for (int i = 0; i < n_neighbor_cells; i++) {
                ...
                result_allcells.push_back({"Ignored", FieldValue{std::move(result_cell)}, "dict"});
            }
            result.push_back({"Neighbor Cells", FieldValue{std::move(result_allcells)}, "list"});
...
_decode_lte_phy_cmlifmr(const char *b, int offset, size_t length,
    ...
    switch (pkt_ver) {
        case 3: {
            ...
            result.push_back({"Detected Cells", FieldValue{std::move(result_allcells)}, "list"});
                    ...
                    if (strcmp(type_name, "Serving_Cell_Measurement_Result") == 0) {
                        switch (subpkt_ver) {
                            case 4: {
                                ...
                                _replace_result(result_subpkt,
                                                             "RSRP Rx[0]", FieldValue{double(RSRP0)});
                                ...
                                _replace_result(result_subpkt,
                                                             "RSRP Rx[1]", FieldValue{double(RSRP1)});
                                ...
                                _replace_result(result_subpkt,
                                                             "RSRQ Rx[0]", FieldValue{double(RSRQ0)});
                                ...
                                _replace_result(result_subpkt,
                                                             "RSRQ Rx[1]", FieldValue{double(RSRQ1)});

                                float RSRQ = float((utemp >> 20) & 1023);
                                RSRQ = RSRQ * 0.0625 - 30.0;
                                _replace_result(result_subpkt,
                                                             "RSRQ", FieldValue{double(RSRQ)});
                                ...
                                _replace_result(result_subpkt,
                                                             "RSSI Rx[0]", FieldValue{double(RSSI0)});

                                float RSSI1 = float((utemp >> 21) & 2047);
                                RSSI1 = RSSI1 * 0.0625 - 110.0;
                                _replace_result(result_subpkt,
                                                             "RSSI Rx[1]", FieldValue{double(RSSI1)});
                                ...
                                SNR0 = SNR0 * 0.1 - 20.0;
                                _replace_result(result_subpkt,
                                                             "FTL SNR Rx[0]", FieldValue{double(SNR0)});

                                float SNR1 = float((utemp >> 9) & 511);
                                SNR1 = SNR1 * 0.1 - 20.0;
                                _replace_result(result_subpkt,
                                                             "FTL SNR Rx[1]", FieldValue{double(SNR1)});
                            ...
                            case 19: {
                                ...
                                _replace_result(result_subpkt,
                                                             "Post IC RSRQ", FieldValue{double(postICRSRQ)});
                            ...
                            case 35: {
                                ...
                                _replace_result(result_subpkt,
                                                             "RSRP Rx[2]", FieldValue{double(RSRP2)});
                                ...
                                _replace_result(result_subpkt,
                                                             "RSRP Rx[3]", FieldValue{double(RSRP3)});
                                ...
                                _replace_result(result_subpkt,
                                                             "Filtered RSRP", FieldValue{double(fRSRP)});
                                ...
                                _replace_result(result_subpkt,
                                                             "RSRQ Rx[2]", FieldValue{double(RSRQ2)});

                                float RSRQ3 = float((utemp >> 20) & 1023);
                                RSRQ3 = RSRQ3 * 0.0625 - 30.0;
                                _replace_result(result_subpkt,
                                                             "RSRQ Rx[3]", FieldValue{double(RSRQ3)});
                                ...
                                _replace_result(result_subpkt,
                                                             "Filtered RSRQ", FieldValue{double(fRSRQ)});
                                ...
                                _replace_result(result_subpkt,
                                                             "RSSI Rx[2]", FieldValue{double(RSSI2)});

                                float RSSI3 = float((utemp >> 11) & 2047);
                                RSSI3 = RSSI3 * 0.0625 - 110.0;
                                _replace_result(result_subpkt,
                                                             "RSSI Rx[3]", FieldValue{double(RSSI3)});
                                ...
                                SNR2 = SNR2 * 0.1 - 20.0;
                                _replace_result(result_subpkt,
                                                             "FTL SNR Rx[2]", FieldValue{double(SNR2)});

                                float SNR3 = float((utemp >> 9) & 511);
                                SNR3 = SNR3 * 0.1 - 20.0;
                                _replace_result(result_subpkt,
                                                             "FTL SNR Rx[3]", FieldValue{double(SNR3)});
        ...
        case 1:
            for (int i = 0; i < n_subpkt; i++) {
                ...
                for (int j = 0; j < n_pilot; j++) {
                    FieldList result_subpkt_cdma_pilot;
                    ...
                    result_subpkt_cdma.push_back({name, FieldValue{std::move(result_subpkt_cdma_pilot)}, "dict"});
                }
                char name[64];
                sprintf(name, "Band_%d", band);
                result_subpkt.push_back({"CDMA", FieldValue{std::move(result_subpkt_cdma)}, "dict"});

                char name2[64];
                sprintf(name2, "Subpacket_%d", i);
                result.push_back({name2, FieldValue{std::move(result_subpkt)}, "dict"});
    ...
    switch (pkt_ver) {
        case 1:
            for (int i = 0; i < n_subpkt; i++) {
                ...
                switch (subpkt_id) {
                    case LtePhyIratType_WCDMA: {
                        ...
                        for (int j = 0; j < n_freq; j++) {
                            FieldList result_subpkt_wcdma_freq;
                            ...
                            for (int k = 0; k < n_cell; k++) {
                                FieldList result_subpkt_wcdma_cell;
                                ...
                                result_subpkt_wcdma_freq.push_back({name, FieldValue{std::move(result_subpkt_wcdma_cell)}, "dict"});
                            }
                            char name[64];
                            sprintf(name, "FrequencyPacket_%d", j);
                            result_subpkt_wcdma.push_back({name, FieldValue{std::move(result_subpkt_wcdma_freq)}, "dict"});
                        }

                        result_subpkt.push_back({"WCDMA", FieldValue{std::move(result_subpkt_wcdma)}, "dict"});
                        break;
                    }
                ...
                result.push_back({name, FieldValue{std::move(result_subpkt)}, "dict"});
...
_decode_lte_pdcp_dl_srb_integrity_data_pdu(const char *b, int offset, size_t length,
    ...
    result.push_back({"Msg", FieldValue{std::vector<uint8_t>((const uint8_t*)(b + offset), (const uint8_t*)(b + offset) + (pdu_length))}, "raw_msg/LTE-PDCP_DL_SRB"});
    return (offset - start) + pdu_length;
...
_decode_lte_pdcp_ul_srb_integrity_data_pdu(const char *b, int offset, size_t length,
    ...
    result.push_back({"Msg", FieldValue{std::vector<uint8_t>((const uint8_t*)(b + offset), (const uint8_t*)(b + offset) + (pdu_length))}, "raw_msg/LTE-PDCP_UL_SRB"});
    return (offset - start) + pdu_length;
}
                    ...
                    switch (subpkt_id) {
                        ...
                        case 1: //DL Config Subpacket
                            if (subpkt_ver == 1) {
                                ...
                                FieldList result_ScellTagInfo;
                                for (int i = 0; i < iNumActiveStag; i++) {
                                    FieldList result_temp;
                                    ...
                                    result_ScellTagInfo.push_back({"Scell Tag Info", FieldValue{std::move(result_temp)}, "dict"});
                                }
                                result_subpkt.push_back({"Scell Tag Info", FieldValue{std::move(result_ScellTagInfo)}, "list"});
                        ...
                        case 3: //RACH Config Subpacket
                            if (subpkt_ver == 1) {
                                ...
                                if(prach_cfg < 16){
                                    ...
                                    std::string temp = "N/A";
                                    FieldValue pystr = FieldValue{std::string(temp.c_str())};
                                    _replace_result(result_subpkt,
                                ...
                                FieldList result_prach_cfg_r13;

                                FieldList result_rsrp_list_size;
                                ...
                                result_prach_cfg_r13.push_back({"Ignored", FieldValue{std::move(result_rsrp_list_size)}, "dict"});

                                FieldList result_rsrp_list;
                                for (int i = 0; i < iListSize; i++) {
                                    ...
                                    offset += _decode_by_fmt(LteMacConfiguration_RachConfigSubpktPayload_rsrp_prach_list_v5,
                                                            ARRAY_SIZE(
                                                                    ...
                                                                    Fmt),
                                                            b, offset, length, result_temp);
                                    result_rsrp_list.push_back({"Ignored", FieldValue{std::move(result_temp)}, "dict"});
                                }
                                result_prach_cfg_r13.push_back({"RSRP Thresh PRACH List", FieldValue{std::move(result_rsrp_list)}, "list"});

                                result_rsrp_list.clear();
                                for (int i = 0; i < 3 - iListSize; i++) {
                                    ...
                                    offset += _decode_by_fmt(LteMacRachTrigger_RachConfigSubpktPayload_hidden_rsrp_prach_list_v5,
                                                            ARRAY_SIZE(
                                                                    ...
                                                                    Fmt),
                                                            b, offset, length, result_temp);
                                    result_rsrp_list.push_back({"Hidden", FieldValue{std::move(result_temp)}, "dict"});
                                }
                                result_prach_cfg_r13.push_back({"Hidden RSRP Thresh PRACH List", FieldValue{std::move(result_rsrp_list)}, "list"});

                                FieldList param_ce_list_size;
                                ...
                                result_prach_cfg_r13.push_back({"Ignored", FieldValue{std::move(param_ce_list_size)}, "dict"});

                                FieldList param_ce_list;
                                for (int i = 0; i < iparamListSize; i++) {
                                    ...
                                    offset += _decode_by_fmt(LteMacConfiguration_RachConfigSubpktPayload_prach_list_v5,
                                                            ARRAY_SIZE(LteMacConfiguration_RachConfigSubpktPayload_prach_list_v5,
                                                                        Fmt),
                                                            b, offset, length, result_temp);
                                    param_ce_list.push_back({"Ignored", FieldValue{std::move(result_temp)}, "dict"});
                                }
                                result_prach_cfg_r13.push_back({"PRACH Param Ce", FieldValue{std::move(param_ce_list)}, "list"});
                                ...
                                result_prach_cfg_r13.push_back({"Hidden Prach Param Ce", FieldValue{std::move(param_ce_list)}, "list"});
                                ...
                                result_prach_cfg_r13.push_back({"Ignored", FieldValue{std::move(result_temp)}, "dict"});

                                int iPRACHCfgR13Present = _search_result_int(result_subpkt, "PRACH Cfg R13 Present");
                                result_subpkt.push_back({
                                        iPRACHCfgR13Present == 0 ? "Hidden PRACH Cfg R13"
                                                                 : "PRACH Cfg R13",
                                        FieldValue{std::move(result_prach_cfg_r13)},
                                        "list"});
                        ...
                        case 4: //LC Config Subpacket
                            if (subpkt_ver == 1) {
                                ...
                                for (int j = 0; j < num_LC; j++) {
                                    ...
                                    result_subpkt.push_back({"added/modified LC", FieldValue{std::move(result_subpkt_LC)}, "dict"});
                                }
                        ...
                        case 14: // All Rach Config SubPacket
                            if (subpkt_ver == 1) {
                                ...
                                for (int j = 0; j < 8; j++) {
                                    ...
                                    result_Cells.push_back({"Ignored", FieldValue{std::move(result_Cell_item)}, "dict"});
                                }
                                result_subpkt.push_back({"Cells", FieldValue{std::move(result_Cells)}, "list"});
                                success = true;
                            }
                            break;
                        case 18:
        ...
        case 1: {
            ...
            for (int i = 0; i < n_subpkt; i++) {
                ...
                if (type_name == NULL) {    // not found
                        ...
                        case 1: {
                            ...
                            for (int j = 0; j < subpkt_nsample; j++) {
                                ...
                                else
                                {
                                    int hdr_count = 0;
                                    int tmp_count = 0;
                                    std::vector<FieldList> mac_hdr_tmp_list;
                                    
                                    FieldList mac_hdr_list;
                                    ...
                                    while (true)
                                    {
                                        ...
                                        _replace_result_int(mac_hdr, "LC ID", iLCID);
                                        ...
                                        if (iLCID >= 0 && iLCID <= 10 && iE != 0) // logical channel
                                        {
                                            ...
                                            _replace_result_int(mac_hdr, "Len", iSDULen);
                                        }

                                        mac_hdr_tmp_list.push_back(std::move(mac_hdr));
                                    ...
                                    {
                                        FieldList &mac_hdr_ce = mac_hdr_tmp_list[hdr_index];
                                        ...
                                        if (iLCID == 30) // L-BSR
                                        {
                                            ...
                                            _replace_result_int(mac_hdr_ce, "BSR LCG 0", iIndex0);
                                            _replace_result_int(mac_hdr_ce, "BSR LCG 1", iIndex1);
                                            _replace_result_int(mac_hdr_ce, "BSR LCG 2", iIndex2);
                                            _replace_result_int(mac_hdr_ce, "BSR LCG 3", iIndex3);

                                            _replace_result_int(mac_hdr_ce, "BSR LCG 0 (bytes)", LteMacULTransportBlock_Mac_CE_BSR_BufferSizeValue[iIndex0]);
                                            _replace_result_int(mac_hdr_ce, "BSR LCG 1 (bytes)", LteMacULTransportBlock_Mac_CE_BSR_BufferSizeValue[iIndex1]);
                                            _replace_result_int(mac_hdr_ce, "BSR LCG 2 (bytes)", LteMacULTransportBlock_Mac_CE_BSR_BufferSizeValue[iIndex2]);
                                            _replace_result_int(mac_hdr_ce, "BSR LCG 3 (bytes)", LteMacULTransportBlock_Mac_CE_BSR_BufferSizeValue[iIndex3]);
                                        }
                                        else if (iLCID == 29 || iLCID == 28) // S/T-BSR
                                        {
                                            ...
                                            if (iLCGId == 0)
                                            {
                                                tmp_count = _decode_by_fmt(LteMacULTransportBlock_Mac_CE_S_T_BSR_LCG0,
                                                       ...
                                                       b, offset, length, mac_hdr_ce);
                                                _replace_result_int(mac_hdr_ce, "BSR LCG 0", iIndex);
                                                _replace_result_int(mac_hdr_ce, "BSR LCG 0 (bytes)", LteMacULTransportBlock_Mac_CE_BSR_BufferSizeValue[iIndex]);
                                            }
                                            else if (iLCGId == 1)
                                            {
                                                tmp_count = _decode_by_fmt(LteMacULTransportBlock_Mac_CE_S_T_BSR_LCG1,
                                                       ...
                                                       b, offset, length, mac_hdr_ce);
                                                _replace_result_int(mac_hdr_ce, "BSR LCG 1", iIndex);
                                                _replace_result_int(mac_hdr_ce, "BSR LCG 1 (bytes)", LteMacULTransportBlock_Mac_CE_BSR_BufferSizeValue[iIndex]);
                                            }
                                            else if (iLCGId == 2)
                                            {
                                                tmp_count = _decode_by_fmt(LteMacULTransportBlock_Mac_CE_S_T_BSR_LCG2,
                                                       ...
                                                       b, offset, length, mac_hdr_ce);
                                                _replace_result_int(mac_hdr_ce, "BSR LCG 2", iIndex);
                                                _replace_result_int(mac_hdr_ce, "BSR LCG 2 (bytes)", LteMacULTransportBlock_Mac_CE_BSR_BufferSizeValue[iIndex]);
                                            }
                                            else if (iLCGId == 3)
                                            {
                                                tmp_count = _decode_by_fmt(LteMacULTransportBlock_Mac_CE_S_T_BSR_LCG3,
                                                       ...
                                                       b, offset, length, mac_hdr_ce);
                                                _replace_result_int(mac_hdr_ce, "BSR LCG 3", iIndex);
                                                _replace_result_int(mac_hdr_ce, "BSR LCG 3 (bytes)", LteMacULTransportBlock_Mac_CE_BSR_BufferSizeValue[iIndex]);
                                            }
                                        }
                                        ...
                                        else if (iLCID == 26) // PHR
                                        {
                                            ...
                                            _replace_result_int(mac_hdr_ce, "PHR Ind", iPHRInd);
                                        }
                                        else if (iLCID == 20) // Recommended bit rate query
                                        {
                                            ...
                                            _replace_result_int(mac_hdr_ce, "LCID (RBRQ)", iRBRQLCID);
                                            _replace_result_int(mac_hdr_ce, "UL/DL", iULorDL);
                                            _replace_result_int(mac_hdr_ce, "Bit Rate", iBitRate);
                                            _replace_result_int(mac_hdr_ce, "Bit Rate Multiplier", iX);
                                        ...
                                        if (iLCID > 10)
                                        {
                                            _replace_result_int(mac_hdr_ce, "Len", iSDULen);
                                        }

                                        mac_hdr_list.push_back({"Ignored", FieldValue{std::move(mac_hdr_ce)}, "dict"});
                                    }



                                    result_subpkt_sample.push_back({"Mac Hdr + CE", FieldValue{std::move(mac_hdr_list)}, "list"});
                                ...
                                result_sample_list.push_back({"Ignored", FieldValue{std::move(result_subpkt_sample)}, "dict"});
                            }
                            success = true;
                            break;
                        }
                    ...
                    if (success) {
                        result_subpkt.push_back({"Samples", FieldValue{std::move(result_sample_list)}, "list"});
                    } else {
                                ...
                                else
                                {
                                    ...
                                    for (uint hdr_index = 0; hdr_index < mac_hdr_tmp_list.size(); ++hdr_index)
                                    {
                                        ...
                                        else if (iLCID == 29) // TA Command
                                        {
                                            ...
                                            _replace_result_int(mac_hdr_ce, "TAG Id", iTAGId);
                                            _replace_result_int(mac_hdr_ce, "TA Command", iTACommand);
                                        }
                ...
                if (type_name == NULL) {    // not found
                    ...
                    switch (subpkt_ver) {
                        case 3: {
                            ...
                            FieldList result_subpkt_allsamples;
                            ...
                            for (int j = 0; j < subpkt_nsample; j++) {
                                ...
                                _replace_result_int(result_subpkt_sample,
                                                                 "Sub FN", iSubFN);
                                _replace_result_int(result_subpkt_sample,
                                                                 "Sys FN", iSysFN);

                                FieldList result_subpkt_sample_alllcids;

                                for (int k = 0; k < num_active_lcid; k++) {
                                    FieldList result_subpkt_sample_lcid;
                                    offset += _decode_by_fmt(
                                    ...
                                    result_subpkt_sample_alllcids.push_back({"Ignored", FieldValue{std::move(result_subpkt_sample_lcid)}, "dict"});
                                }
                                result_subpkt_sample.push_back({"LCIDs", FieldValue{std::move(result_subpkt_sample_alllcids)}, "list"});

                                result_subpkt_allsamples.push_back({"Ignored", FieldValue{std::move(result_subpkt_sample)}, "dict"});
                            }
                            result_subpkt.push_back({"Samples", FieldValue{std::move(result_subpkt_allsamples)}, "list"});
                    ...
                    if (success) {
                        result_allpkts.push_back({"MAC Subpacket", FieldValue{std::move(result_subpkt)}, "dict"});
                    } else {
                ...
                if (type_name == NULL) {    // not found
                    ...
                    switch (subpkt_ver) {
                        case 1: // UL Tx Stats SubPacket version 1
                        {
                            ...
                            result_subpkt.push_back({"Sample", FieldValue{std::move(result_subpkt_sample)}, "dict"});
                            success = true;
                            break;
                        }
        ...
        case 1: {
            ...
            for (int i = 0; i < n_subpkt; i++) {
                ...
                if (subpkt_id == 69 && subpkt_ver == 1) {
                    ...
                    FieldList result_ReleasedRB;
                    for (int j = 0; j < num_ReleasedRB; j++) {
                        FieldList result_ReleasedRB_item;
                        ...
                        result_ReleasedRB.push_back({"Ignored", FieldValue{std::move(result_ReleasedRB_item)}, "dict"});
                    }
                    result_subpkt.push_back({"Released RBs", FieldValue{std::move(result_ReleasedRB)}, "list"});
                    ...
                    FieldList result_AddedModifiedRB;
                    for (int j = 0; j < num_AddedModifiedRB; j++) {
                        FieldList result_AddedModifiedRB_item;
                        ...
                        (void) _map_result_field_to_name(result_AddedModifiedRB_item,
                                                         ...
                                                         ARRAY_SIZE(
                                                                 ...
                                                                 ValueName),
                                                         "(MI)Unknown");
                        result_AddedModifiedRB.push_back({"Ignored", FieldValue{std::move(result_AddedModifiedRB_item)}, "dict"});
                    }
                    result_subpkt.push_back({"Added/Modified RBs", FieldValue{std::move(result_AddedModifiedRB)}, "list"});
                    ...
                    FieldList result_ActiveRB;
                    for (int j = 0; j < num_ActiveRB; j++) {
                        FieldList result_ActiveRB_item;
                        ...
                        (void) _map_result_field_to_name(result_ActiveRB_item,
                                                         ...
                                                         ARRAY_SIZE(LteRlcUlConfigLogPacket_Subpkt_ActiveRB_RBType,
                                                                    ValueName),
                                                         "(MI)Unknown");
                        FieldValue pystr = FieldValue{std::string("RLCUL CFG")};
                        _replace_result(result_ActiveRB_item,
                                                               "RLCUL CFG", pystr);

                        _replace_result_int(result_ActiveRB_item,
                                                         "SN Length", 10);

                        result_ActiveRB.push_back({"Ignored", FieldValue{std::move(result_ActiveRB_item)}, "dict"});
                    }
                    result_subpkt.push_back({"Active RBs", FieldValue{std::move(result_ActiveRB)}, "list"});
                    ...
                    for (int j = 0; j < num_ActiveRB; j++) {
                        ...
                        (void) _map_result_field_to_name(result_ActiveRB_item,
                                                         ...
                                                         ARRAY_SIZE(LteRlcDlConfigLogPacket_Subpkt_ActiveRB_RBType,
                                                                    ValueName),
                                                         "(MI)Unknown");
                        FieldValue pystr = FieldValue{std::string("RLCDL CFG")};
                        _replace_result(result_ActiveRB_item,
                                                               "RLCDL CFG", pystr);
...
static int _decode_lte_rlc_ul_am_all_pdu_subpkt(const char *b, int offset,
                                                size_t length, FieldList &result) {
    int start = offset;
    int pkt_ver = _search_result_int(result, "Version");
    int n_subpkt = _search_result_int(result, "Number of Subpackets");
        ...
        case 1: {
            ...
            for (int i = 0; i < n_subpkt; i++) {
                ...
                if (subpkt_id == 70 && subpkt_ver == 3) {
                    ...
                    FieldValue pystr = FieldValue{std::string(strEnabledPDU.c_str())};
                    _replace_result(result_subpkt,
                                                           "Enabled PDU Log Packets", pystr);

                    int n_pdu = _search_result_int(result_subpkt,
                                                   "Number of PDUs");
                    FieldList result_pdu;
                    for (int j = 0; j < n_pdu; j++) {
                        ...
                        if (DCLookAhead < 16) {
                            // Type = Control
                            _replace_result(result_pdu_item,
                                                         "PDU TYPE", FieldValue{std::string("RLCUL CTRL")});
                            // Modify ACK_SN
                            ...
                            _replace_result(result_pdu_item,
                                                         "SN", FieldValue{std::string(strAckSN.c_str())});
                            // Update other info
                            offset += _decode_by_fmt(
                                    ...
                                    b, offset, length, result_pdu_item);
                            _replace_result(result_pdu_item,
                                                         "cpt", FieldValue{std::string("STATUS(0)")});
                            ...
                            if (iLoggedBytes > 0) {
                                ...
                                FieldList result_pdu_nack;
                                for (int indexNack = 0; indexNack < numNack; indexNack++) {
                                    FieldList result_pdu_nack_item;
                                    if (iAllign == 0) {
                                        iAllign = 1;
                                        offset += _decode_by_fmt(
                                    ...
                                    result_pdu_nack.push_back({"Ignored", FieldValue{std::move(result_pdu_nack_item)}, "dict"});
                                }
                                result_pdu_item.push_back({"RLC CTRL NACK", FieldValue{std::move(result_pdu_nack)}, "list"});
                            }
                        } else {
                            // Type = DATA
                            _replace_result(result_pdu_item,
                                                         "PDU TYPE", FieldValue{std::string("RLCUL DATA")});
                            // Update other info
                            ...
                            offset += _decode_by_fmt(
                                    ...
                                    b, offset, length, result_pdu_item);
                            _replace_result(result_pdu_item,
                                                         "RF", FieldValue{std::string((strRF.c_str()))});
                            _replace_result(result_pdu_item,
                                                         "P", FieldValue{std::string((strP.c_str()))});
                            _replace_result(result_pdu_item,
                                                         "FI", FieldValue{std::string((strFI.c_str()))});
                            _replace_result(result_pdu_item,
                                                         "E", FieldValue{std::string((strE.c_str()))});
                            ...
                            if (strE == "1") {
                                ...
                                FieldList result_pdu_li;
                                for (int indexLI = 0; indexLI < numLI; indexLI++) {
                                    FieldList result_pdu_li_item;
                                    if (iAllign == 0) {
                                        iAllign = 1;
                                        offset += _decode_by_fmt(
                                    ...
                                    result_pdu_li.push_back({"Ignored", FieldValue{std::move(result_pdu_li_item)}, "dict"});
                                }
                                result_pdu_item.push_back({"RLC DATA LI", FieldValue{std::move(result_pdu_li)}, "list"});
                            }
                        }
                        offset += iLoggedBytes;
                        result_pdu.push_back({("RLCUL PDU["
                                                               + SSTR(i) + "]").c_str(), FieldValue{std::move(result_pdu_item)}, "dict"});
                    }
                    result_subpkt.push_back({"RLCUL PDUs", FieldValue{std::move(result_pdu)}, "list"});
                ...
                if ((subpkt_id == 65 && subpkt_ver == 3) || (subpkt_id == 65 && subpkt_ver == 4)) {
                    ...
                    for (int j = 0; j < n_pdu; j++) {
                        ...
                        if (DCLookAhead < 16) {
                            // Type = Control
                            _replace_result(result_pdu_item,
                                                         "PDU TYPE", FieldValue{std::string("RLCDL CTRL")});
                            _replace_result(result_pdu_item,
                                                         "Status", FieldValue{std::string("PDU CTRL")});
                            // Modify ACK_SN
                            std::string strAckSN = "ACK_SN = ";
                            int iAckSN = DCLookAhead * 64 + iNonDecodeSN / 4;
                            strAckSN += SSTR(iAckSN);
                            ...
                            // Type = DATA
                            _replace_result(result_pdu_item,
                                                         "PDU TYPE", FieldValue{std::string("RLCDL DATA")});
                            _replace_result(result_pdu_item,
                                                         "Status", FieldValue{std::string("PDU DATA")});
                            // Update other info
                            std::string strRF = "", strP = "", strFI = "",
                                    strE = "";
                            if ((DCLookAhead) & (1 << (6))) {
                        ...
                        offset += iLoggedBytes;
                        result_pdu.push_back({("RLCDL PDU["
                                                               + SSTR(i) + "]").c_str(), FieldValue{std::move(result_pdu_item)}, "dict"});
                    }
                    result_subpkt.push_back({"RLCDL PDUs", FieldValue{std::move(result_pdu)}, "list"});
                } else {
                ...
                if (subpkt_id == 3 && subpkt_ver == 2) {
                    ...
                    for (int j = 0; j < num_Cells; j++) {
                        ...
                        offset += _decode_by_fmt(LteMacRachTrigger_RachConfigSubpktPayload_v4_cell,
                                                 ...
                                                 b, offset, length, result_Cell_item);
                        _replace_result_int(result_Cell_item,
                                                                   "Preamble Format", 0);
                    ...
                    (void) _map_result_field_to_name(result_subpkt, "Group chosen",
                                                     ...
                                                     ARRAY_SIZE(LteMacRachTrigger_RachReasonSubpkt_GroupChosen,
                                                                ValueName),
                                                     "(MI)Unknown");
                    const FieldValue *temp=_search_result(result_subpkt,"Maching ID");
                    long long tempLong = temp ? _field_value_as_int(*temp) : 0;
                    ...
                    FieldValue pystr1 = FieldValue{std::string(MachingId.c_str())};
                    _replace_result(result_subpkt,
                                                           "Maching ID", pystr1);


                    std::string strRachContention = "Contention Based RACH procedure";
                    FieldValue pystr = FieldValue{std::string(strRachContention.c_str())};
                    _replace_result(result_subpkt,
            ...
            for (int i = 0; i < n_subpkt; i++) {
                ...
                if ((subpkt_id == 6 && subpkt_ver == 2)) {
                    ...
                    if ((iRachMsgBMasks) & (1 << 2)) {
                        ...
                        result_subpkt.push_back({"Msg1", FieldValue{std::move(result_subpkt_msg1)}, "dict"});
                    }
                    if ((iRachMsgBMasks) & (1 << 1)) {
                        // Msg2
                        FieldList result_subpkt_msg2;
                        ...
                        (void) _map_result_field_to_name(result_subpkt_msg2,
                                                         ...
                                                         ARRAY_SIZE(LteMacRachAttempt_Subpkt_Msg2_Result,
                                                                    ValueName),
                                                         "False");
                        result_subpkt.push_back({"Msg2", FieldValue{std::move(result_subpkt_msg2)}, "dict"});
                    }
                    if ((iRachMsgBMasks) & (1)) {
                        // Msg3
                        FieldList result_subpkt_msg3;
                        ...
                        int iGrantBytes = _search_result_int(result_subpkt_msg3,
                                                             "Grant");
                        FieldList result_MACPDUs;
                        for (int j = 0; j < iGrantBytes + 1; j++) {
                            FieldList result_MACPDU_item;
                            offset += _decode_by_fmt(
                                    LteMacRachAttempt_Subpkt_Msg3_MACPDU,
                                    ARRAY_SIZE(LteMacRachAttempt_Subpkt_Msg3_MACPDU,
                                               Fmt),
                                    b, offset, length, result_MACPDU_item);
                            result_MACPDUs.push_back({"Ignored", FieldValue{std::move(result_MACPDU_item)}, "dict"});
                        }
                        // add pdu list
                        result_subpkt_msg3.push_back({"MAC PDUs", FieldValue{std::move(result_MACPDUs)}, "list"});
                        // add Msg3 dict
                        result_subpkt.push_back({"Msg3", FieldValue{std::move(result_subpkt_msg3)}, "dict"});
                    }
                    ...
                    if ((iRachMsgBMasks) & (1)) {
                        ...
                        FieldValue pystr1 = FieldValue{std::string(GrantRaw.c_str())};
                        _replace_result(result_subpkt_msg3,
                                                            "Grant Raw", pystr1);
            ...
            for (int i = 0; i < n_subpkt; i++) {
                ...
                if (subpkt_id == 194 && subpkt_ver == 1) {
                    ...
                    for (int j = 0; j < num_PDU; j++) {
                        ...
                        result_PDU.push_back({"Ignored", FieldValue{std::move(result_PDU_item)}, "dict"});
                    }
                    result_subpkt.push_back({"PDCP UL Data PDU", FieldValue{std::move(result_PDU)}, "list"});
                } else {
                ...
                if (subpkt_id == 196 && subpkt_ver == 2) {
                    ...
                    for (int j = 0; j < num_RB; j++) {
                        ...
                        (void) _map_result_field_to_name(result_RB_item,
                                                         ...
                                                         "(MI)Unknown");
                        result_RB.push_back({"Ignored", FieldValue{std::move(result_RB_item)}, "dict"});
                    }
                    result_subpkt.push_back({"RBs", FieldValue{std::move(result_RB)}, "list"});
            ...
            for (int i = 0; i < n_subpkt; i++) {
                ...
                if (subpkt_id == 194 && subpkt_ver == 1) {
                    ...
                    for (int j = 0; j < num_PDU; j++) {
                        ...
                        FieldValue pystr = FieldValue{std::string(strType.c_str())};
                        _replace_result(result_PDU_item,
                                                     "type", pystr);

                        result_PDU.push_back({"Ignored", FieldValue{std::move(result_PDU_item)}, "dict"});
                    }
                    result_subpkt.push_back({"PDCP DL Ctrl PDU", FieldValue{std::move(result_PDU)}, "list"});
...
static int _decode_lte_pdcch_phich_indication_report_payload(const char *b,
    ...
    switch (pkt_ver) {
        case 5: {
            ...
            _replace_result_int(result, "Duplex Mode",
                                                       iDuplexMode);
            int num_record = _search_result_int(result, "Number of Records");
            ...
            for (int i = 0; i < num_record; i++) {
                ...
                FieldList result_pdcch;
                for (int j = 0; j < iNumPdcch; j++) {
                    FieldList result_pdcch_item;
                    ...
                    _replace_result_int(result_pdcch_item,
                    ...
                    result_pdcch.push_back({"Ignored", FieldValue{std::move(result_pdcch_item)}, "dict"});
                }
                result_record_item.push_back({"PDCCH Info", FieldValue{std::move(result_pdcch)}, "list"});
        ...
        case 25: {
            ...
            for (int i = 0; i < num_record; i++) {
                ...
                FieldList result_phich;
                for (int j = 0; j < iNumPhich; j++) {
                    FieldList result_phich_item;
                    ...
                    _replace_result_int(result_phich_item,
                                                     "Cell Index", iCellIndex);
                    _replace_result_int(result_phich_item,
                                                     "PHICH Included", iPhichIncluded);
                    ...
                    result_phich.push_back({"Ignored", FieldValue{std::move(result_phich_item)}, "dict"});
                }
                result_record_item.push_back({"PHICH", FieldValue{std::move(result_phich)}, "list"});
        ...
        case 42: {
            ...
            _replace_result_int(result, "UL DL Config",
                                             iULDLConfig);
            ...
            for (int i = 0; i < num_record; i++) {
                ...
                for (int j = 0; j < 16; j++) {
                    ...
                    _replace_result_int(result_pdcch_item, "S0 Index", iresulttemp);

                    iresulttemp = (utemp >> 5) & 0x1f;
                    _replace_result_int(result_pdcch_item, "S1 Index", iresulttemp);

                    iresulttemp = (utemp >> 10) & 0x1f;
                    _replace_result_int(result_pdcch_item, "S2 Index", iresulttemp);

                    iresulttemp = (utemp >> 15) & 0x1f;
                    _replace_result_int(result_pdcch_item, "S3 Index", iresulttemp);

                    iresulttemp = (utemp >> 20) & 0x1;
                    _replace_result_int(result_pdcch_item, "Msleep", iresulttemp);

                    iresulttemp = (utemp >> 21) & 0x1;
                    _replace_result_int(result_pdcch_item, "Usleep", iresulttemp);

                    iresulttemp = (utemp >> 22) & 0x1f;
                    _replace_result_int(result_pdcch_item, "uSleep Duration", iresulttemp);

                    iresulttemp = (utemp >> 27) & 0x1;
                    _replace_result_int(result_pdcch_item, "Fake Pdcch Sf", iresulttemp);

                    iresulttemp = (utemp >> 28) & 0x1;
                    _replace_result_int(result_pdcch_item, "Is Ul Dropped", iresulttemp);

                    iresulttemp = (utemp >> 29) & 0x1;
                    _replace_result_int(result_pdcch_item, "Interf Active", iresulttemp);
                ...
                _replace_result_int(result_p3, "Force Send PDCCH Ind", iresulttemp);

                result_record_item.push_back({"Ignored", FieldValue{std::move(result_p3)}, "dict"});
...
_nr_rrc_reconf_complete_to_ul_dcch(const char *b, int pdu_length) {
    std::vector<uint8_t> ul_dcch_msg(pdu_length + 1);
        ...
        if (type_name == NULL) {    // not found
            ...
            if (pdu_number == 0x0a) {
                ...
                result.push_back({"Msg",
                    FieldValue{_nr_rrc_reconf_complete_to_ul_dcch(b + offset, pdu_length)},
                    type_str});
            } else {
                result.push_back({"Msg",
                    FieldValue{std::vector<uint8_t>(
                        (const uint8_t*)(b + offset),
                        (const uint8_t*)(b + offset) + pdu_length)},
                    type_str});
            }
            return (offset - start) + pdu_length;
        }
...
_decode_modem_debug_msg(const char *b, int offset, size_t length,
    ...
    if (version == '\x79') {
        ...
        if (argc == 2 && tmp_argv[0] == 0x2e3bbbed) {
            ...
            result.push_back({"Msg", FieldValue{std::string(res.c_str(), res.size())}, ""});
...
// is_log_packet / is_debug_packet / is_custom_packet moved to
// bytes_proto/qualcomm/consts.cpp (shared with the export manager).

void
on_demand_decode (const char *b, size_t length, LogPacketType type_id, FieldList &result)
{
    int offset = 0;
    switch (type_id) {
...
decode_log_packet(const char *b, size_t length, bool skip_decoding) {
    ...
    if(skip_decoding){
		...
		if(diff >= 1){
			...
			return FieldList();  // sampling gate: caller sets ok = false
		}
	        else{
			//pass
		}
    ...
    LogPacketType type_id = (LogPacketType) _map_result_field_to_name(
            ...
            LogPacketTypeID_To_Name_n,
            "Unsupported");

    /*
    if (skip_decoding) {    // skip further decoding

        result.push_back({"Msg", FieldValue{std::vector<uint8_t>((const uint8_t*)(b + offset), (const uint8_t*)(b + offset) + (length-offset))}, "raw_msg/skip_decoding"});
        return result;
    }
    */
...
decode_custom_packet (const char *b, size_t length) {
    ...
    _replace_result(result, "type_id", FieldValue{std::string("Custom_Packet")});
...
void
decode_custom_packet_payload (const char *b, size_t length, FieldList &result)
{
    FieldValue pystr = FieldValue{std::string(b, length)};
    _replace_result(result, "Msg", pystr);
}
    ...
    for (int i = 0; i < n_fmt; i++) {
        ...
        switch (fmt[i].type) {
            case UINT: {
                ...
                if (fmt[i].len <= 4)
                    result.push_back({fmt[i].field_name, FieldValue{int64_t(ii)}, ""});
                else
                    result.push_back({fmt[i].field_name, FieldValue{int64_t(iiii)}, ""});
            ...
            case QCDM_TIMESTAMP: {
                ...
                long long seconds = (long long)(double(iiii) / PER_SECOND);
...
decode_log_packet_modem(const char *b, size_t length, bool skip_decoding) {
    ...
    // Keep entries [1, 4) — remove the duplicate "len1" field
    // (was: result = PyList_GetSlice(result, 1, 4))
    {
        FieldList sliced;
        for (size_t k = 1; k < result.size() && k < 4; ++k)
            sliced.push_back(std::move(result[k]));
        result = std::move(sliced);
    }
