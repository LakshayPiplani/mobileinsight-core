...
static int _decode_gsm_rcrm_payload (const char *b,
    ...
    _replace_result_int(result, "Serving BCCH ARFCN",
            iServingBCCH);
    _replace_result_int(result, "Serving BCCH Band",
            iServingBCCHBand);
    ...
    _replace_result_int(result, "Serving PBCCH ARFCN",
            iServingPBCCH);
    _replace_result_int(result, "Serving PBCCH Band",
            iServingPBCCHBand);
    ...
    _replace_result(result, "Serving RX Level Average", FieldValue{double(fRx)});
    ...
    for (int i = 0; i < num_record; i++) {
        ...
        _replace_result_int(result_record_item, "BCCH ARFCN",
                iBCCH);
        _replace_result_int(result_record_item, "BCCH Band",
                iBCCHBand);
        ...
        _replace_result_int(result_record_item, "PBCCH ARFCN",
                iPBCCH);
        _replace_result_int(result_record_item, "PBCCH Band",
                iPBCCHBand);
        ...
        float fRx = temp - 110.5;
        _replace_result(result_record_item, "RX Level Average", FieldValue{double(fRx)});
    ...
    result.push_back({"Neighboring 6 Strongest Cells", FieldValue{std::move(result_record)}, "list"});
    return offset - start;
}
