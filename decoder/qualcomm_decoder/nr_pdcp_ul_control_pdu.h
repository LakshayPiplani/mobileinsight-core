    ...
    case 1:
        {
        	...
        	//ControlPDU                    
            FieldList result_ControlPDU;
            for (int i = 0; i < num_ControlPDU; i++) {
            	//ControlPDUItem
                FieldList result_ControlPDU_item;
                ...
                _replace_result_int(result_ControlPDU_item,
                        "RB ID", iID);
                _replace_result_int(result_ControlPDU_item,
                ...
                int value=tmp&0x3ff;
                _replace_result_int(result_SysTime,
                         "FN", value);

				result_ControlPDU_item.push_back({"Sys Time", FieldValue{std::move(result_SysTime)}, "dict"});
				...
				result_ControlPDU.push_back({name, FieldValue{std::move(result_ControlPDU_item)}, "dict"});	
            }

                  
 			//ControlPDU > result
            result.push_back({"Control PDU", FieldValue{std::move(result_ControlPDU)}, "list"});
            return offset - start; 
        }
    default:
